            ...
            get
            {
                if (moveUpClickCommand == null)
                    moveUpClickCommand = new SDCommand<string>(
                        new Action<string>(e =>
                        {
                            DesktopItem preDesktopItem = null;
                            for (int i = 0; i < DesktopManager.GetInstance().DesktopItemList.Count; i++)
                            {
                                ...
                                {
                                    if (preDesktopItem == null) continue;

                                    if(DesktopManager.GetInstance().DesktopItemList[i].Config == Item.Config)
                                    {
                                        DesktopItem temp = DesktopManager.GetInstance().DesktopItemList[i].Clone();
                                        DesktopManager.GetInstance().DesktopItemList[i] = preDesktopItem.Clone();
                                        preDesktopItem = temp;
                                        break;
                                    }
                                    else
                                    {
                                        preDesktopItem = DesktopManager.GetInstance().DesktopItemList[i];
                                    }
                                }
                            }
                        }), null);
